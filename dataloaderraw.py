...
class DataLoaderRaw():
    ...
    def __init__(self, opt):
        ...
        print('coco_json :', self.coco_json)
        self.folder_path = opt.get('folder_path', '')

        self.batch_size = opt.get('batch_size', 1)
        print('opt_batch_size', opt.get('batch_size', 1))
        print('self_batch_size', self.batch_size)
        self.seq_per_img = 1
        ...
        print('coco_json_length :', len(self.coco_json))
        if len(self.coco_json) > 0:
            ...
            for root, dirs, files in os.walk(self.folder_path, topdown=False):
                for file in sorted(files):
                    ...
                    if isImage(fullpath):
                        self.files.append(fullpath)
                        self.ids.append(str(file.split('.')[0])) # just order them sequentially, origin str(n)
                        n = n + 1

        self.N = len(self.files)
        print('DataLoaderRaw found ', self.N, ' images')
        print('list_ids :', self.ids)

        self.iterator = 0

    def get_batch(self, split, batch_size=None):
        print('s_batch_size :', self.batch_size)
        ...
        for i in range(batch_size):
            ...
            print('i :', i)
            #print(self.files[0])
            ri_next = ri + 1 # origin : 1
            print('ri_next :', ri_next)
            ...
            print('img_1 path :', self.files[ri])
            #img_2 = skimage.io.imread(self.files[ri+1])
            #print('img_2 path :', self.files[ri+1])
            #img_diff = skimage.io.imread(self.files[ri+2])
            #print('img_diff path :', self.files[ri+2])
            i#mg = (img_1 + img_2 + img_diff*3) / 5
